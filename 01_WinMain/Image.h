...
public://렌더링 관련 ~
	...
	void ScaleRender(HDC hdc, int x, int y, int width, int height, int tempX, int tempY, int tempWidth, int tempHeight);
	//크기 조정해주는 프레임 렌더링
...
public:// 접근 관련~
	...
	inline void SetWidth(int width)const {  mImageBuffer->width = width; }
};
